...
char *arbre::binaire(std::string stringBinaire, int & tailleString) {
    ...
    if (tailleString%8 != 0) {
        ...
        for (int i = 0; i < 256; i++)
            if (stringPlusLong.length() < codes[i].length()) 
                // on trouve le code le plus long pour completer le dernier octet du string à encoder
                stringPlusLong = codes[i];                          
    ...
    // pour encoder tout le string 
    while (compteur < tailleString) {                               
